...
def changeFrame(button_frame, choice, cash_input, change_input):
    ...
    method_of_payment = tk.Label(change_frame, text='Method of Payment', font=custom_font, foreground='white', bg='#dd0329')

    cost = tk.Label(change_frame, text='Cost', font=custom_font,foreground='white', bg='#dd0329')

    change = tk.Label(change_frame, text='Change', font=custom_font, foreground='white', bg='#dd0329') 

    payment_entry = ttk.Combobox(change_frame, font=custom_font,
    width=24, state='readonly', textvariable=choice, justify='right')
    payment_entry['values'] = ('', 'K Pay', 'Wave', 'CB Pay', 'AYA Pay')
    payment_entry.current(0)

    cost_entry = tk.Entry(change_frame, font=custom_font, width=24, 
    textvariable=cash_input, justify='right')
    cost_entry.insert(0, '0')

    change_entry = tk.Entry(change_frame, font=custom_font, width=24, 
